...
__global__ void initMatrixGPU(float *X, int N, curandState *state){
	int i, seed=1337;
...
{	
	...
	// Launch init kernel
	initMatrixGPU<<<2*numSM, THREADSPERBLOCK, 0, streams[0]>>>(A,nX*nY,state);
	initMatrixGPU<<<2*numSM, THREADSPERBLOCK, 0, streams[1]>>>(B,nX*nY, state);
